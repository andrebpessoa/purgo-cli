...
export class CleanUI {
  ...
  startSearching(): void {
    this.spinner = ora(chalk.cyan('Searching for artifacts to clean...')).start();
  }
  ...
  showNothingToClean(): void {
    this.spinner?.succeed(chalk.green('No targets found. The project is already clean!'));
  }
  ...
  showTargets(targets: TargetWithSize[]): void {
    ...
    for (const target of targets) {
      console.log(`  - ${target.path} ${chalk.gray(`(${prettyBytes(toBytes(target.size))})`)}`);
    }
  ...
  showDryRunNotice(): void {
    console.log(
      boxen(chalk.bold(chalk.yellow('--dry-run enabled. No files will be deleted.')), {
        padding: 1,
        margin: 1,
        borderStyle: 'round',
      })
  ...
  stopProgress(success: boolean = true, message?: string): void {
    ...
    if (success) {
      this.progressSpinner.succeed(chalk.green(message || 'Operation completed'));
    } else {
