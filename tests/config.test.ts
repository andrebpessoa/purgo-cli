...
const existsSyncMock = mock((_p: string) => true);
...
const importConfig = async (suffix: string = String(Math.random())) =>
	await import(`../src/config?${suffix}`);
...
describe("loadConfig", () => {
	test("merges global + workspace + package.json with extends", async () => {
		...
		existsSyncMock.mockImplementation(
			(p: string) =>
				p === globalPath ||
				p.endsWith("extends-a.json") ||
				p.endsWith("extends-b.json"),
		);
		...
		} as unknown as CosmiconfigResult);
		...
		expect(
			Array.isArray(loaded.config.targets) ||
				typeof loaded.config.targets === "undefined",
		).toBe(true);
		expect(
			Array.isArray(loaded.config.ignore) ||
				typeof loaded.config.ignore === "undefined",
		).toBe(true);
	...
	test("detects cycle in extends and throws error", async () => {
		...
		loadMap.set(globalPath, {
			config: { extends: ["a.json"] },
			filepath: globalPath,
		});
		loadMap.set("/home/.config/purgo-cli/a.json", {
			config: { extends: ["b.json"] },
			filepath: "/home/.config/purgo-cli/a.json",
		});
		loadMap.set("/home/.config/purgo-cli/b.json", {
			config: { extends: ["a.json"] },
			filepath: "/home/.config/purgo-cli/b.json",
		});

		await expect(
			loadConfig({ projectRoot, globalConfigPath: globalPath }),
	...
	test("extends invalid by schema throws error", async () => {
		...
		existsSyncMock.mockImplementation((_p: string) => true);
		loadMap.set(globalPath, {
			config: { extends: ["bad.json"] },
			filepath: globalPath,
		});
		loadMap.set("/home/.config/purgo-cli/bad.json", {
			config: { targets: 123 },
			filepath: "/home/.config/purgo-cli/bad.json",
		} as unknown as CosmiconfigResult);

		await expect(
			loadConfig({ projectRoot, globalConfigPath: globalPath }),
		).rejects.toThrow(/Invalid extends/i);
	});
	...
	test("resolves relative extends path correctly", async () => {
		...
		existsSyncMock.mockImplementation((_p: string) => true);
		loadMap.set(globalPath, {
			config: { extends: ["../shared/rel.json"] },
			filepath: globalPath,
		});
		loadMap.set("/home/.config/shared/rel.json", {
			config: { targets: ["rel"] },
			filepath: "/home/.config/shared/rel.json",
		});
	...
	test("hooks makes deep merge preserving unoverridden values", async () => {
		...
		existsSyncMock.mockImplementation((_p: string) => true);
		loadMap.set(globalPath, {
			config: { hooks: { preClean: "global-pre", postClean: "global-post" } },
			filepath: globalPath,
		});
		searchMap.set(projectRoot, {
			config: { hooks: { preClean: "workspace-pre" } },
			filepath: `${projectRoot}/.purgo.json`,
		});
	...
	test("confirms merge order global -> workspace -> package.json", async () => {
		...
		existsSyncMock.mockImplementation((_p: string) => true);
		loadMap.set(globalPath, {
			config: { targets: ["g"], ignore: ["ig-g"] },
			filepath: globalPath,
		});
		searchMap.set(projectRoot, {
			config: { targets: ["w"], ignore: ["ig-w"] },
			filepath: `${projectRoot}/.purgo.json`,
		});
		loadMap.set(`${projectRoot}/package.json`, {
			config: { targets: ["p"] },
