...
const globMock = mock(async (_patterns: string[], _opts: unknown) => {
	return ["node_modules", "dist"];
});
const getFolderSizeMock = mock(async (_p: string) => 1024 * 1024); // 1 MB per target
const promptsMock = mock(async (_q: unknown) => ({ confirm: true }));
...
const chalkMock = new Proxy({}, { get: () => (v: unknown) => String(v) });
...
const importIndex = async (suffix: string = String(Math.random())) =>
	await import(`../src/index?${suffix}`);
...
describe("cleanProject", () => {
	...
	test("toBytes handles number, bigint, objects and fallback", async () => {
		...
		getFolderSizeMock.mockImplementation(async (p: string): Promise<number> => {
			const key = p.split(/\\|\//).pop() as string;
			return toBytes(sizes[key]);
		});
	...
	test("targets precedence: CLI prevails over config and default", async () => {
		mock.module("../src/config", () => ({
			loadConfig: mock(async () => ({
				config: { targets: ["from-config"], ignore: ["**/build"] },
			})),
		}));
	...
	test("ignore merge includes fixed rule and config ones", async () => {
		...
		globMock.mockImplementationOnce(
			async (_patterns: string[], opts: unknown) => {
				const o = opts as { ignore: string[] };
				expect(Array.isArray(o.ignore)).toBe(true);
				expect(o.ignore).toContain("**/node_modules/**/node_modules");
				expect(o.ignore).toContain("**/build");
				expect(o.ignore).toContain("**/.cache");
				return [];
			},
		);
